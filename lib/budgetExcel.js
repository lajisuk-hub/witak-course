...
function detailLine({ name, unit, count, months, ratio, total, hasRatio }) {
  ...
  // 세입의 인건비 보조금 줄만 칸이 밀려 있다(개월 자리 = 지원율, 그 뒤 칸 = 개월).
  // 개월 자리 값이
  //   · 1보다 작으면          → 사람이 쓸 수 없는 개월 수이므로 지원율(0.8, 0.0475 …)
  //   · 딱 1이고 뒤 칸이 숫자    → 지원율 100%, 뒤 칸이 진짜 개월
  //   · 딱 1이고 뒤 칸이 비었거나 '월 20시간' 같은 설명 → 그냥 **1개월짜리 항목**
  //   · 1보다 크면              → 밀리지 않은 보통 줄(연장반 등)
  // 마지막 경우가 "× 100%" 로 잘못 나오던 것을 바로잡음 (2026-09-10)
  const ratioIsNumber =
    typeof ratio === 'number' || /^\s*\d+(\.\d+)?\s*$/.test(String(ratio ?? ''));
  if (hasRatio && mm > 0 && mm <= 1 && (mm < 1 || ratioIsNumber)) {
    rate = mm;
    mm = num(ratio);
    note = '';
  }

  // 세출의 4대보험·퇴직적립금 줄은 **인원 칸에 요율**(0.0475 등)이 들어온다.
  // 그대로 두면 "302,779,200원 × 0.0475명" 이라는 이상한 산출기초가 되므로
  // 사람 수가 될 수 없는 1 미만 소수는 요율로 보고 "× 4.75%" 로 적는다.
  if (!hasRatio && !mm && cnt > 0 && cnt < 1) {
    rate = cnt;
    cnt = 0;
  }

  // 인원 칸에 "1회" 처럼 단위가 함께 적혀 있으면 그 단위를 그대로 쓴다.
  // (예비비처럼 사람 수가 아니라 횟수로 세는 항목 — 숫자만 있으면 예전처럼 '명')
  const countUnit = String(count ?? '').replace(/[\d.,\s]/g, '').trim() || '명';

  // 요율은 반올림하면 4.75% 가 5% 로 나와 산출기초가 틀려 보인다 → 소수점 셋째까지
  const pct = (r) => String(Number((r * 100).toFixed(3)));

  const parts = [name];
  if (num(unit)) parts.push(`${money(unit)}원`);
  if (cnt) parts.push(`× ${cnt}${countUnit}`);
  if (mm) parts.push(`× ${mm}월`);
  if (rate) parts.push(`× ${pct(rate)}%`);
  if (note) parts.push(`(${note})`);

  // 예비비처럼 단가·인원·개월 없이 금액만 적힌 줄은 산출기초가 이름뿐이라
  // 심사에서 "산출내역이 없다"는 지적을 받는다. 한 번에 쓰는 돈으로 보고
  // "2,777,149원 × 1회 = 2,777천원" 으로 풀어 쓴다.
  // (예전에 받아 두신 엑셀도 이 규칙으로 되살아난다)
  if (!num(unit) && !cnt && !mm && !rate && num(total)) {
    return `${name} ${money(total)}원 × 1회 = ${thousand(total)}천원`;
  }
...
const REMAP = {
  ...
  //   · 기타 인건비는 앱이 150/151 로 적지만 서식의 목 코드는 131
  //     (안 바꾸면 대체교사 같은 금액이 한글 예산서에서 통째로 빠진다 — 2026-09-10)
  expense: { 130: '131', 150: '131', 151: '131' },
};

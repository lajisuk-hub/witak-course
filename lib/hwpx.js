...
import { applyToHeader, applyToSection, isEmptySetting } from './docSetting';
import { findMarkedRanges } from './markers';
...
export async function buildHwpx({
  ...
  // ── 원장님이 넣어 둔 [[표시]] 로 항목 구간을 찾는다 ──
  // 표시가 있으면 그것을 먼저 쓰고, 없으면 예전 방식(조각 지도)으로 넘어간다.
  const ranges = findMarkedRanges(raw);
  const marked = {};
  if (ranges) ranges.forEach((r) => (marked[r.id] = raw.slice(r.start, r.end)));

  // ── 표지 (샘플 표지를 그대로 쓰되 이름만 바꿔 넣는다) ──
  let cover = marked.cover || cut(SAMPLE_MAP.coverBlock);
  cover = cover
  ...
  items.forEach((it) => {
    ...
    // 원장님이 [[표시]] 를 넣어 둔 항목이면 그 구간을 그대로 쓴다
    if (marked[it.matchId] && it.matchId !== 'cover') {
      done.add(it.matchId);
      body += marked[it.matchId];
      return;
    }
  ...
  let sectionXml = skeletonHead + cover + toc + body + '</hs:sec>';

  // ── 지자체가 정한 문서 설정(글꼴·크기·줄간격·여백)을 반영한다 ──
  let headerXml = null;
  if (!isEmptySetting(setting)) {
    if (onProgress) onProgress('지자체가 정한 글꼴·여백을 맞추는 중입니다...');
    const rawHeader = await zip.file('Contents/header.xml').async('string');
    headerXml = applyToHeader(rawHeader, setting, bodyStyle);
    sectionXml = applyToSection(sectionXml, setting);
  }
  ...
  const skip = new Set(['mimetype', 'Contents/section0.xml']);
  if (headerXml) skip.add('Contents/header.xml');
  ...
  if (headerXml) out.file('Contents/header.xml', headerXml, { compression: 'DEFLATE' });

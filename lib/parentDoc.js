...
export const PARENT_PROGRAMS = [
  { key: 'father', name: '아빠 참여수업', detail: '아빠와 함께하는 놀이·체험 활동을 연 2회 이상 운영한다(토요일·휴일 활용). 신체놀이·목공·요리·바깥놀이 등 아빠-자녀 활동으로 아버지의 양육 참여를 확대한다.' },
  { key: 'grand', name: '조부모 참여수업', detail: '조부모를 초청하여 세대공감 프로그램을 연 1~2회 운영한다. 전래놀이·옛이야기·손주와의 활동으로 세대 간 정서 교류와 조부모 양육 자원을 활용한다.' },
  { key: 'observe', name: '부모 참관수업', detail: '부모가 자녀의 보육활동에 직접 참여·참관하는 공개수업을 반별 연 1~2회 운영한다. 자녀의 어린이집 생활을 이해하고 신뢰를 형성한다.' },
  { key: 'family', name: '가족 참여 행사', detail: '가족 운동회·가족 소풍·나들이 등 가족 참여 행사를 연 1~2회 운영한다. 가족 간 유대와 어린이집 공동체 의식을 높인다.' },
  { key: 'talent', name: '부모 재능기부 수업', detail: '부모의 직업·특기를 활용한 재능기부 수업을 수시로 운영한다. 다양한 직업 세계와 특기 체험을 제공하고 부모의 주도적 참여를 지원한다.' },
  { key: 'show', name: '부모 초청 발표회', detail: '연말 학예발표회·작품전시회에 부모를 초청한다(연 1회). 영유아의 한 해 성장을 부모와 함께 나눈다.' },
  { key: 'event', name: '생일·명절 축하 행사', detail: '부모 초청 생일잔치와 명절(설·추석) 체험 행사를 운영한다. 전통문화 체험과 가족 참여로 정서적 유대를 높인다.' },
  { key: 'volunteer', name: '부모 봉사·모니터링', detail: '부모 급식 모니터링·도서·환경 봉사와 부모모니터링단을 운영한다. 보육과정에 부모가 참여하고 운영의 투명성을 높인다.' },
];
...
const INTRO =
  ' 부모와 가족이 어린이집 활동에 직접 참여하는 프로그램을 운영하여, 부모를 보육의 동반자로 세우고 가정과 어린이집의 연계를 강화한다. 위탁심사 요구사항인 3가지 이상 참여 프로그램을 운영한다.';
...
function buildSection(section, selected) {
  ...
  const chosen = PARENT_PROGRAMS.filter((p) => selected.includes(p.key));
  const block = headingPara('(4) 부모·가족 참여 프로그램', INTRO) + tablePara(chosen);
  return section.slice(0, insertAt) + block + section.slice(insertAt);
}

/**
 * @param {object} o
 * @param {string[]} o.selected  고른 참여 프로그램 key 배열
 * @param {string} o.phone
 * @param {string} o.city
 * @param {string} o.student
 * @param {Function} [o.onProgress]
 */
export async function buildParentDoc({ selected, phone, city, student, onProgress }) {
  ...
  if (onProgress) onProgress('고르신 참여 프로그램을 넣는 중입니다...');
  ...
  const chosen = PARENT_PROGRAMS.filter((p) => selected.includes(p.key)).map((p) => p.name);
  return { blob, name: fileName({ city, student, docName: '학부모참여수업' }), programs: chosen };
}

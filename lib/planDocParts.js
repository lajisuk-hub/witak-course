...
import { polishBlock } from './planPolish';
import { YEAR_PLAN, YEAR_INTRO } from './yearPlan';
...
/** "나. 연간 …" 큰 제목의 가나다 번호를 바꾼다 (앞에 새 항목을 넣었으므로) */
function relabel(chunk, letter) {
  return chunk.replace(/<hp:t>(\s*)[가-하]\./, `<hp:t>$1${letter}.`);
}
...
/** 나. 연령별 발달 특성과 보육 방향 */
function traitsSection(ages, styles, tblSt) {
  let xml = para('', styles.body);
  xml += para('나. 연령별 발달 특성과 보육 방향', styles.sect);
  xml += para(TRAITS_INTRO, styles.body);
  xml += table(
    ['연 령', '발달 특성', '놀이 · 보육 지원 방향', '교사의 상호작용 중점'],
    ages
      .filter((a) => AGE_TRAITS[a.key])
      .map((a) => [a.label, AGE_TRAITS[a.key].dev, AGE_TRAITS[a.key].care, AGE_TRAITS[a.key].talk]),
    [7, 20, 19, 18],
    tblSt
  );
  return xml;
...
/** 다. 보육과정 편성·운영·평가 절차 */
function processSection(styles, tblSt) {
  let xml = para('', styles.body);
  xml += para('다. 보육과정 편성·운영·평가 절차', styles.sect);
  xml += para(PROCESS_INTRO, styles.body);
  xml += para('1) 연간–월간–주간–일일 계획의 연계', styles.head);
  xml += table(
    ['단 계', '무엇을 정하는가', '누가 · 언제', '근거 자료'],
    PROCESS_LEVELS.map((p) => [p.k, p.what, p.who, p.src]),
    [10, 22, 15, 17],
    tblSt
  );
  xml += para('2) 보육과정 편성 절차', styles.head);
  xml += table(
    ['절 차', '운영 내용'],
    PROCESS_STEPS.map((p) => [p.k, p.v]),
    [11, 53],
    tblSt
  );
  return xml;
...
/** 바. 연령별 주간보육계획안 */
function weeklyBlock(age, no, styles, tblSt) {
  const w = WEEKLY[age.key];
  if (!w) return '';
  let xml = para(`${no}) ${age.short} 주간보육계획안 (3월 1주 예시)`, styles.head);
  xml += para(`▣ 이번 주 놀이 : ${w.theme}   ▣ 주간 목표 : ${w.goal}`, styles.body);
  xml += table(
    ['요일', '이어지는 놀이 흐름', '교사의 지원', '일상생활 · 안전'],
    w.days.map((d) => [d.d, d.play, d.support, d.life]),
    [5, 21, 21, 17],
    tblSt
  );
  xml += table(['주간 평가 및 다음 주 반영'], [[w.review]], [64], tblSt, []);
  xml += para('', styles.body);
  return xml;
}

/** 아. 연장보육 운영 계획 */
function extendedSection(styles, tblSt) {
  let xml = para('', styles.body);
  xml += para('아. 연장보육 운영 계획', styles.sect);
  xml += para(EXTENDED_INTRO, styles.body);
  xml += table(
    ['구 분', '운영 내용'],
    EXTENDED_RUN.map((r) => [r.k, r.v]),
    [12, 52],
    tblSt
  );
  xml += para('◦ 연장보육 시간대별 운영', styles.body);
  xml += table(
    ['시 간', '운영 내용'],
    EXTENDED_TIME.map((r) => [r.t, r.v]),
    [12, 52],
    tblSt
  );
  return xml;
...
/** 자. 보육과정 기록·평가 서식 */
function formsSection(styles, tblSt) {
  let xml = para('', styles.body);
  xml += para('자. 보육과정 기록·평가 서식', styles.sect);
  xml += para(FORMS_INTRO, styles.body);

  xml += para(FORM_OBSERVE.title, styles.head);
  xml += table(FORM_OBSERVE.head, FORM_OBSERVE.rows, [10, 20, 18, 16], tblSt);

  xml += para(FORM_MONTHLY_EVAL.title, styles.head);
  xml += table(FORM_MONTHLY_EVAL.head, FORM_MONTHLY_EVAL.rows, [12, 30, 22], tblSt);

  xml += para(FORM_SELF_EVAL.title, styles.head);
  xml += table(FORM_SELF_EVAL.head, FORM_SELF_EVAL.rows, [10, 26, 14, 14], tblSt);
  return xml;
}

/** 라. 연간 놀이계획안 — 새로 쓴 완성본 (연령 하나) */
function yearBlock(age, no, styles, tblSt) {
  const y = YEAR_PLAN[age.key];
  if (!y) return '';
  let xml = para(`${no}) ${age.short} 연간 놀이계획안`, styles.head);
  xml += para(`▣ 보육목표 : ${y.goal}`, styles.body);
  xml += table(
    ['월', '생활주제', '예상되는 놀이 흐름', '놀이 속 배움 (관련 영역)', '연계 (행사·안전·인성)'],
    y.months.map((m) => [m.m, m.theme, m.play, m.area, m.link]),
    [4, 10, 22, 13, 15],
    tblSt,
    [0, 1]
  );
 ...
 */
export function buildPlanSection(sectionXml, ageKeys, headerXml, opts) {
  const useSampleYear = (opts || {}).yearPlan === 'sample';
  ...
  // ── 나. 연령별 발달 특성 / 다. 편성·운영·평가 절차 (새로 넣는 내용) ──
  let out = traitsSection(wanted, styles, tblSt);
  out += processSection(styles, tblSt);
  ...
  if (useSampleYear) {
    // (선택) 원장님 샘플의 표를 그대로 쓰고 글자만 다듬는다
    out += relabel(xml.slice(paras[yearHead][0], paras[yearHead][1]), '라');
    const yearIntro = paras[yearHead + 1];
    if (
      yearIntro &&
      !texts[yearHead + 1].match(/^\d+\)/) &&
      !xml.slice(yearIntro[0], yearIntro[1]).includes('<hp:tbl ')
    ) {
      out += xml.slice(yearIntro[0], yearIntro[1]);
    }
    out += para(
      '※ 연간 계획은 3월에 시작하여 이듬해 2월에 마치며, 표의 1월·2월은 이듬해를 뜻한다.',
      styles.body
    );
    wanted.forEach((age) => {
      ...
      out += polishBlock(renumber(block, no), age.key);
    });
  } else {
    // (기본) 열두 달을 모두 채운 완성본을 새로 만들어 넣는다
    out += para('라. 연간 표준보육과정 기반 놀이계획안', styles.sect);
    out += para(YEAR_INTRO, styles.body);
    wanted.forEach((age) => {
      if (!YEAR_PLAN[age.key]) {
        missing.push(age.short);
        return;
      }
      no += 1;
      used.push(age.short);
      out += yearBlock(age, no, styles, tblSt);
    });
  }

  if (!used.length) {
    throw new Error('고른 연령의 연간놀이계획안을 만들지 못했습니다.');
  }
  ...
  const inDoc = (age) => used.includes(age.short);

  // ── 마. 월간보육계획안 ──
  out += para('', styles.body);
  out += para('마. 연령별 월간보육계획안', styles.sect);
  ...
  // ── 바. 주간보육계획안 ──
  out += para('바. 연령별 주간보육계획안', styles.sect);
  out += para(WEEKLY_INTRO, styles.body);
  let wno = 0;
  wanted.forEach((age) => {
    if (!inDoc(age)) return;
    wno += 1;
    out += weeklyBlock(age, wno, styles, tblSt);
  });

  // ── 사. 하루 일과 및 보육일지 ──
  out += para('사. 연령별 하루 일과 및 보육일지', styles.sect);
  ...
  // ── 아. 연장보육 / 자. 기록·평가 서식 ──
  out += extendedSection(styles, tblSt);
  out += formsSection(styles, tblSt);

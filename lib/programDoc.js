...
// 원장님 **특색프로그램 한글 서식(forms/special.hwpx)** 을 채워 내려받는다.
//
...
export async function buildProgramDoc({ file, phone, city, student, onProgress }) {
  ...
  if (onProgress) onProgress('특색프로그램 서식을 불러오는 중입니다...');
  const ticket = await fetch(`/api/sample?kind=special&phone=${encodeURIComponent(phone)}`);
  ...
  if (!res.ok) throw new Error('특색프로그램 서식을 받지 못했습니다');
  ...
  return {
    blob,
    name: fileName({ city, student, docName: '특색프로그램' }),

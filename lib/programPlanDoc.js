 ...
 * @param {string} [o.yearPlan] 'sample' 이면 원장님 원본 연간계획안 표를 쓴다 (기본은 새 완성본)
 * @param {string} o.phone
 * @param {string} [o.city]
 * @param {string} [o.student]
 * @param {Function} [o.onProgress]
 */
export async function buildProgramPlanDoc({ ages, phone, city, student, yearPlan, onProgress }) {
  const JSZip = await loadJSZip();
  ...
  const { xml, used, missing } = buildPlanSection(section, ages, header, { yearPlan });

...
function buildSection(section, selected) {
  ...
  // 글자(번호·소개문)를 바꾸면 문단에 들어 있던 줄 정보(linesegarray)와 어긋나
  // 한글이 "손상/변조" 오류를 내며 못 연다 → 줄 정보를 빼면 한글이 열 때 다시 계산한다.
  return (prefix + middle + suffix).replace(/<hp:linesegarray>[\s\S]*?<\/hp:linesegarray>/g, '');
}
